...
class VideoMkvClusterAlignTest {
    ...
    @Test
    fun alignClusterStart_usesForwardClusterWhenCandidateBeforeFirstMarker() {
        val probeStart = 80_000_000L
        val clusterAt = 80_500_000L
        val candidate = 80_400_000L
        val probe = ByteArray(1_048_576) { 0x00 }
        val rel = (clusterAt - probeStart).toInt()
        probe[rel] = 0x1F.toByte()
        probe[rel + 1] = 0x43
        ...
        assertEquals(
            clusterAt,
            VideoMkvClusterAlign.alignClusterStart(probe, probeStart, candidate),
        )
    }

    @Test
    fun findFirstClusterOffsetInProbe_returnsEarliestMarker() {
        ...
        assertEquals(probeStart + 1, VideoMkvClusterAlign.findFirstClusterOffsetInProbe(probe, probeStart))
    }
